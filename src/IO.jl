...
function initialize(file::String)
    ...
    #outfolder
    indx = findfirst(x->contains(x,"outfile"),input)
    if indx==nothing
        throw(ArgumentError("missing parameter outfile !"))
    end
    outfile = String(split(split(input[indx],"=")[2])[1])

    pSim = SimulationParameters(nx=nx,ny_avg=ny_avg,bmin=bmin,bmax=bmax,nb=nb,theta_in=theta_in,eta=eta,nk=nk,outfile=outfile)
    ...
    #D
    indx = findfirst(x->contains(x,"D_yrz"),input)
    if indx==nothing
        push!(missing_input,"D_yrz")
...
mu_aux  = 0.0\nQ  = []\nPcdw = 0.0\ng_tilde = 0.0\nt_orth = 0.0\nac = 1.0")
    ...
    #Compute bare FS ?
    indx = findfirst(x->contains(x,"BareFSswitch"),input)
    if indx==nothing
        BareFSswitch = true
    else
        BareFSswitch = parse(Bool,split(input[indx],"=")[2])
    end

    #Compute DOS ?
    indx = findfirst(x->contains(x,"DOSswitch"),input)
    if indx==nothing
        DOSswitch = true
    else
        DOSswitch = parse(Bool,split(input[indx],"=")[2])
    end

    return pSim,pLayer,BareFSswitch, DOSswitch
end
...
write bare FS into outfile.FS
"""
function saveFS(FS,pSim)
    f = open(pSim.outfile*".FS","w")
    write(f,"kx                  ky                 ")
    for orb in FS.orbs
        write(f,orb*"                  ")
    end
...
save DOS into outfile.DOS
"""
function saveDOS(dos,pSim)
    f = open(pSim.outfile*".DOS","w")
    write(f,"Number of averaging ky-points = "*string(pSim.ny_avg)*"\n")
    write(f,"1/B      DOS(ny_1) ... DOS(ny_N) Avg(DOS)\n")
    for ib = 1:pSim.nb
        write(f,string(1/pSim.bgrid[ib])*" ")
        for iy = 1:pSim.ny_avg+1
